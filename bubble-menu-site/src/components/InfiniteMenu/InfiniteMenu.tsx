...
void main() {
    ...
    bool isActive = (uActiveInstanceId >= 0 && vInstanceId == uActiveInstanceId);
    if (!isActive) {
...
class InfiniteGridMenu {
  ...
  private render(): void {
    ...
    gl.uniform1i(this.discLocations.uActiveInstanceId!, this.activeInstanceId);
  ...
  private onControlUpdate(deltaTime: number): void {
    ...
    if (!this.control.isPointerDown) {
      ...
      this.activeInstanceId = nearestVertexIndex;
      const itemIndex = nearestVertexIndex % Math.max(1, this.items.length);
      this.onActiveItemChange(itemIndex);
      const snapDirection = vec3.normalize(vec3.create(), this.getVertexWorldPosition(nearestVertexIndex));
      this.control.snapTargetDirection = snapDirection;
    } else {
      this.activeInstanceId = -1;
...
const defaultItems: MenuItem[] = [
  { image: 'https://picsum.photos/900/900', link: 'https://google.com/', title: '', description: '' }
];
...
const InfiniteMenu: FC<InfiniteMenuProps> = ({ items = [], scale = 1.0 }) => {
  ...
  const handleCloseModal = () => setShowRecordModal(false);
  ...
  return (
    <div className="infinite-menu-wrap" style={{ position: 'relative', width: '100%', height: '100%' }}>
      ...
      {showRecordModal && activeItem && createPortal(
        <div
          className="infinite-menu-modal-overlay"
          onClick={handleCloseModal}
          onKeyDown={e => e.key === 'Escape' && handleCloseModal()}
          role="dialog"
          aria-modal="true"
          aria-label="Запись блога"
        >
          <div className="infinite-menu-modal" onClick={e => e.stopPropagation()}>
            <button
              type="button"
              className="infinite-menu-modal-close"
              onClick={handleCloseModal}
              aria-label="Закрыть"
            >
              ×
            </button>
            <div className="infinite-menu-modal-image-wrap">
              <img
                key={activeItem.image}
                src={encodeURI(activeItem.image)}
                alt=""
                className="infinite-menu-modal-image"
                style={{ filter: 'none' }}
              />
            </div>
            <h3 className="infinite-menu-modal-title">{activeItem.title}</h3>
            <p className="infinite-menu-modal-description">{activeItem.description}</p>
            {activeItem.link && activeItem.link !== '#' && (
              <button type="button" className="infinite-menu-modal-link-btn" onClick={handleModalLink}>
                Перейти по ссылке
              </button>
            )}
          </div>
