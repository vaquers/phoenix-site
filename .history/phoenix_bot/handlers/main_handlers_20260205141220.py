...
async def start(message: Message):
    ...
    if is_admin:
        await message.answer(f"{time_text}, {name}", reply_markup=await home_keyboard_for_admin())
    else:
        await message.answer(f"Извините, ботом может пользоваться только администратор", reply_markup=await home_keyboard())
